...
function orientationFromVariantNode(node) {
  const image = node?.product?.featuredImage;
...
function packageEntryForSizeName({registry, specs, sizeName}) {
  if (!sizeName) return null;

  const fromRegistry = resolveShippingBoxForSizeName(registry, sizeName);
  const spec = specBySizeName(specs, sizeName);
  ...
  if (
    ...
    return {
      ...
      sizeKey: fromRegistry.sizeKey ?? slugify(sizeName),
...
function applyOrientationToBox(packageEntry, orientation) {
  ...
  return {
    length: packageEntry.width,
    width: packageEntry.length,
    height: packageEntry.height,
  };
}
...
function resolvePackageForLineItem({catalog, specs, registry, item, variantNode}) {
  ...
  const packageEntry = parsed?.sizeName
    ? packageEntryForSizeName({registry, specs, sizeName: parsed.sizeName})
    : null;
  const catalogVariant = parsed ? findCatalogVariant(catalog, parsed) : null;
  const orientation = variantNode
    ? orientationFromVariantNode(variantNode)
    : 'vertical';
  ...
  const dimensions = applyOrientationToBox(packageEntry, orientation);
