...
export async function linkFulfillmentToPickup(fulfillmentRecordId) {
  ...
  const hasLabel = Boolean(fulfillmentFields[ff.shippingLabel]);
  const existingNotes = pickup.fields?.[pf.notes] ?? '';
  let easypostPickup = null;

  if (!hasLabel) {
    easypostPickup = {
      skipped: true,
      reason: 'Fulfillment has no shipping label yet — set In Progress first to purchase a label',
    };
  } else if (!SHIPPING.isConfigured()) {
    easypostPickup = {
      skipped: true,
      reason: 'EasyPost is not configured (set EASYPOST_API_KEY)',
    };
  } else if (pickupNotesContainEasypostPickup(existingNotes)) {
    easypostPickup = {
      skipped: true,
      reason: 'EasyPost pickup already scheduled for this Airtable pickup row',
    };
  } else if (when) {
    easypostPickup = await scheduleEasypostCarrierPickup({
      fulfillmentRecordId,
      scheduledAt: when,
      pickupReference: `pickup-${pickup.id}`,
    });

    if (easypostPickup.action === 'scheduled') {
      await updatePickupRecord($, pickup.id, {
        [pf.notes]: appendEasypostPickupNote(existingNotes, easypostPickup),
