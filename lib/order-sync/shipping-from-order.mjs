...
export function resolveEasypostCarrierPreference(order, pkg = null) {
  ...
  const defaultCarriers = (process.env.EASYPOST_PREFERRED_CARRIERS?.trim() ?? 'Purolator,CanadaPost,UPS,FedExDefault')
    .split(',')
    .map((part) => part.trim())
    .filter(Boolean);
...
function applyOversizedCarrierPreference(pkg, preference) {
  ...
  return {
    ...preference,
    carriers: carriersForOversizedParcel(preference.carriers),
    oversized: true,
    oversizedDetails: {
  ...
  if (carriers.length) {
    const preferred = available.filter((rate) =>
      carriers.some((carrier) =>
        String(rate.carrier ?? '')
          .toLowerCase()
          .includes(String(carrier).toLowerCase()),
      ),
    );
    if (preferred.length) {
      pool = preferred;
