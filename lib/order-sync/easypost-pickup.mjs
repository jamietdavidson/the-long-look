...
function pickupSupportedCarrier(carrier) {
  ...
  if (/purolator/.test(normalized)) {
    return false;
  }
  return /canadapost|ups|fedex|dhl/.test(normalized);
}
...
export async function scheduleEasypostCarrierPickup(params) {
  ...
  if (!pickupSupportedCarrier(carrier)) {
    return {
      skipped: true,
      reason:
        carrier.toLowerCase().includes('purolator')
          ? 'Purolator does not support scheduled pickup via EasyPost — drop off at a Purolator centre'
          : `Carrier ${carrier} does not support scheduled pickup via EasyPost`,
      carrier,
