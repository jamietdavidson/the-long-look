...
export async function shopifyAdminGraphql($, query, variables = {}) {
  ...
  if (!token) {
    throw new Error('SHOPIFY_ACCESS_TOKEN is required for Shopify Admin API requests');
  }
...
/**
 * List Shopify orders updated since an ISO timestamp (REST Admin API).
 * @param {string} updatedAtMinIso
 */
export async function listShopifyOrdersUpdatedSince(updatedAtMinIso) {
  const config = shopifyAdminConfig();
  const token = await resolveToken();
  if (!token) {
    throw new Error('SHOPIFY_ACCESS_TOKEN is required to list Shopify orders');
  }

  const orders = [];
  let url =
    `https://${config.shop}.myshopify.com/admin/api/${SHOPIFY.apiVersion}/orders.json` +
    `?status=any&limit=50&updated_at_min=${encodeURIComponent(updatedAtMinIso)}`;
  ...
  while (url) {
    ...
    if (!response.ok) {
      throw new Error(
        `Shopify orders list failed (${response.status}): ${JSON.stringify(payload)}`,
      );
    }

    orders.push(...(payload.orders ?? []));
    url = parseLinkHeaderNextUrl(response.headers.get('link'));
  }
