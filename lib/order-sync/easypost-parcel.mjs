...
export function parseOversizedCarrierList() {
  return (process.env.EASYPOST_OVERSIZED_CARRIERS?.trim() ?? 'Purolator,UPS,FedExDefault')
    .split(',')
    .map((part) => part.trim())
    .filter(Boolean);
}
...
/**
 * Carriers that accept oversized parcels in Canada. Canada Post max L+g is 118".
 * @param {string[]} carriers
 */
export function carriersForOversizedParcel(carriers) {
  const oversized = parseOversizedCarrierList();
  if (oversized.length) {
    return oversized;
  }

  return carriers.filter((carrier) => !/canadapost/i.test(carrier));
}
