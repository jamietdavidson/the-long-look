...
function buildPurchaseInput({fulfillmentOrderId, pkg, preferredRate}) {
...
async function downloadLabelPdf(url) {
  ...
  const response = await fetch(url, {
    headers: config.token ? {'X-Shopify-Access-Token': config.token} : {},
  });
  if (!response.ok) {
...
export async function createShopifyShippingLabelsForOrder(order, packagesOverride = null) {
  ...
  for (const [index, pkg] of packages.entries()) {
    ...
    const started = await purchaseLabel($, purchaseInput);
    if (!started?.id) {
