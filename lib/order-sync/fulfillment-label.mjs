...
import {createShopifyShippingLabelForPackage} from './shopify-shipping.mjs';
...
function shippingPaidPerFulfillment(order, fulfillmentCount) {
  if (!fulfillmentCount) return null;
  const paid = Number(
  ...
  return Math.round((paid / fulfillmentCount) * 100) / 100;
}
...
export async function createLabelForFulfillment(fulfillmentRecordId) {
  ...
  if (!SHIPPING.isConfigured()) {
    return {
      ...
      fulfillmentRecordId,
      reason:
        'Shopify Shipping is not configured (set SHOPIFY_ACCESS_TOKEN with write_orders and fulfillment scopes)',
    };
  }
  ...
  try {
    const label = await createShopifyShippingLabelForPackage(shopifyOrder, picked.pkg);
    if (label.skipped) {
      return {
        action: 'skipped',
        fulfillmentRecordId,
    ...
    const filename = `shopify-dhl-label-${orderName}-fulfillment-${fulfillmentRecordId}.pdf`;
    const labelUrl = await uploadLabelPdfForAirtable($, label.labelPdf, filename);
    const shippingCost = shippingPaidPerFulfillment(shopifyOrder, allFulfillments.length);

    await updateFulfillmentRecord($, fulfillmentRecordId, {
      [ff.shippingLabel]: labelUrl,
      ...(shippingCost != null ? {[ff.shippingCost]: shippingCost} : {}),
    });
