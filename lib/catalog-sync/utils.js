...
/** Remove Shopify prints, artists, and collections whose Airtable rows no longer exist. */
export async function syncCatalogDeletions($, airtable, shopify, dryRun) {
  const printPrune = await pruneOrphanedPrints($, airtable, shopify, dryRun);

  const [artistIds, collectionIds, artistObjects, collectionObjects] = await Promise.all([
    listTableRecordIds($, airtable, 'artists'),
    listTableRecordIds($, airtable, 'collections'),
    listMetaobjectsByType($, shopify, SHOPIFY.metaobjectTypes.artist),
    listMetaobjectsByType($, shopify, SHOPIFY.metaobjectTypes.collection),
  ]);
  ...
  for (const metaobject of artistObjects) {
    if (!metaobject.airtableRecordId || artistIdSet.has(metaobject.airtableRecordId)) {
      continue;
    }
  ...
  for (const metaobject of collectionObjects) {
    if (!metaobject.airtableRecordId || collectionIdSet.has(metaobject.airtableRecordId)) {
      continue;
    }
  ...
  const variantRemoved = await pruneOrphanedProductVariants($, airtable, shopify, dryRun);
  ...
  return {
    removed: {
      ...
      variants: variantRemoved.removed,
    },
    count:
      printRemoved.length +
      removedArtists.length +
      removedCollections.length +
      variantRemoved.count,
    printPrune,
    variantPrune: variantRemoved,
  };
}
