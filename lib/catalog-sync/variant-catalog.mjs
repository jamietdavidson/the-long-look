...
export async function pruneOrphanedProductVariants($, airtable, shopify, dryRun) {
  ...
  const removed = await mapWithConcurrency(products, concurrency, async (product) => {
  ...
  const pruned = removed.filter(Boolean);
  ...
  return {
    ...
    skipped: pruned.filter((entry) => entry.status === 'skipped').length,
  };
}
...
export async function syncCatalogToAllProducts($, shopify, catalog, {dryRun = false} = {}) {
  ...
  const results = await mapWithConcurrency(products, concurrency, (product) =>
    reconcileProductVariantCatalog($, shopify, {
      productId: product.id,
      catalog,
      dryRun,
    }),
  ...
  return {productCount: products.length, products, results};
}
