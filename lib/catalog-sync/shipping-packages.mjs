...
import {
  ...
  resolveVariantShippingWeightLb,
...
export async function ensureShippingPackages($, shopify, catalog, {dryRun = false} = {}) {
  ...
  for (const spec of specs) {
    ...
    try {
      const isDefault =
        spec.packagingType === 'box' && spec.rank === specs.find((s) => s.packagingType === 'box')?.rank;
...
export async function assignProductVariantShippingPackages(
  ...
  for (const catalogVariant of catalog) {
    ...
    const packageKey =
      catalogVariant.frame === 'No Frame' ? `${sizeKey}-tube` : sizeKey;
    ...
    const measurement = {
      weight: {
        value: resolveVariantShippingWeightLb(catalogVariant),
        unit: 'POUNDS',
      },
    };
    if (shopifyPackageId) {
      measurement.shippingPackageId = shopifyPackageId;
    }
...
export function resolveShippingBoxForSizeName(registry, sizeName, {frame = null} = {}) {
  ...
  const entry =
    ...
    Object.values(packages).find(
      (pkg) =>
        pkg.sizeName?.toLowerCase() === sizeName.toLowerCase() &&
        (frame === 'No Frame'
          ? pkg.packagingType === 'tube'
          : pkg.packagingType !== 'tube'),
    ) ??
    null;
